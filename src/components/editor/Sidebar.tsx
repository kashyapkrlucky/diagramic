...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <div
      className={`absolute right-0 top-0 bottom-0 w-80 bg-white border-l h-[calc(100vh-70px)] flex flex-col border-gray-200 shadow-2xl transition-all duration-300 ease-in-out ${
      ...
      <section className="px-6 py-4 overflow-hidden flex-1">
        {currentTab === tabs[0] && (
          ...
          >
            <>
              ...
              <div className="space-y-2 overflow-y-auto hide-scrollbar" style={{ height: "calc(100vh - 275px)" }}>
                {nodes.length > 0 ? (
                  nodes.map((node) => (
                    <AccordionItem
                      key={node.id}
        ...
        {currentTab === tabs[1] && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xs font-semibold text-gray-900">JSON Data</h2>
              <div className="flex items-center gap-2">
                <button
            ...
            <div className="relative">
              <textarea
                className="w-full h-94 p-4 hide-scrollbar text-xs font-mono bg-gray-900 text-green-400 border border-gray-200 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
